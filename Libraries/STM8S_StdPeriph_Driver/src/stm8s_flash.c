...
void FLASH_EraseByte(uint32_t Address)
{
  ...
  /* Erase byte */
  *(uint8_t*) Address = FLASH_CLEAR_BYTE; 
}
...
void FLASH_ProgramWord(uint32_t Address, uint32_t Data)
{
  ...
  *(((uint8_t*)Address) + 1) = *((uint8_t*)(&Data)+1); 
  /* Write one byte*/    
  *(((uint8_t*)Address) + 2) = *((uint8_t*)(&Data)+2); 
  /* Write one byte - from higher address*/
  *(((uint8_t*)Address) + 3) = *((uint8_t*)(&Data)+3); 
}
...
void FLASH_EraseOptionByte(uint16_t Address)
{
  ...
  else
  {
    ...
    *((uint8_t*)Address) = FLASH_CLEAR_BYTE;
    *((uint8_t*)((uint16_t)(Address + (uint16_t)1 ))) = FLASH_SET_BYTE;
  }
  FLASH_WaitForLastOperation(FLASH_MEMTYPE_PROG);
...
uint16_t FLASH_ReadOptionByte(uint16_t Address)
{
  ...
  value_optbyte = *((uint8_t*)Address); /* Read option byte */
  value_optbyte_complement = *(((uint8_t*)Address) + 1); /* Read option byte complement */
...
IN_RAM(void FLASH_EraseBlock(uint16_t BlockNum, FLASH_MemType_TypeDef FLASH_MemType))
{
  ...
  pwFlash = (uint8_t *)(MemoryAddressCast)(startaddress + ((uint32_t)BlockNum * FLASH_BLOCK_SIZE));
#elif defined(STM8S105) || defined(STM8S005) || defined(STM8S103) || defined(STM8S003) || \
  defined (STM8S903) || defined (STM8AF626x) || defined (STM8AF622x)
    pwFlash = (uint32_t *)(MemoryAddressCast)(startaddress + ((uint32_t)BlockNum * FLASH_BLOCK_SIZE));
                        ...
                        FLASH_ProgramMode_TypeDef FLASH_ProgMode, uint8_t *Buffer))
{
  ...
  {
    *((uint8_t*) (MemoryAddressCast)startaddress + Count) = ((uint8_t)(Buffer[Count]));
  }
}
